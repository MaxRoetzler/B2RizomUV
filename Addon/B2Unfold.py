...
def B2Unfold_AutoFunction():
	meshCheck()
	path, obj, nmList, inList = B2Unfold_Utilites()
	...
	B2Unfold_ImportFunction(path, obj, nmList, inList)


def B2Unfold_ManuelExport():
	meshCheck()
	path, obj, nmList, inList = B2Unfold_Utilites()
	...
	if platform == "darwin":
		l = os.listdir(unfold3DPath)
		appName = (str(l).strip("[]")).strip("'")
		subprocess.Popen([unfold3DPath + appName, '-cfi', path + 'Unwrap.lua'])
	elif platform == "win32":
		subprocess.Popen([unfold3DPath + 'unfold3d.exe', '-cfi', path + 'Unwrap.lua'])


def B2Unfold_ManuelImport():
	path = manList[0]
	obj = manList[1]
	nmList = manList[2]
	inList = manList[3]
...
def B2Unfold_ImportFunction(Path, Object, Names, ImportList):
	# ---------------------------------------- Importing ---------------------------------------
	# region Importing
...
def B2Unfold_Settings():
	...
	bpy.types.Scene.manualTab = BoolProperty \
			(
			name="Manual",
			description="Send and receive option for manual unwrapping in Unfold3D",
			default=True,
			update=tgl_manual
		)
	...
	bpy.types.Scene.autoTab = BoolProperty \
			(
			name="Auto",
			description="Automation options for Blender to Unfold3D. Setup respective settings in Blender and have Unfold3D execute them automatically",
			default=False,
			update=tgl_auto
		)
...
class B2Unfold(bpy.types.Operator):
	...
	def execute(self, context):
		...
		set_distoControlOps()
		set_settings()
		B2Unfold_AutoFunction()
		return {'FINISHED'}

class B2UnfoldManuelExport(bpy.types.Operator):
	bl_idname = "b2unfoldmanual.link"
	bl_label = "Manual Link"

	def execute(self, context):
		B2Unfold_ManuelExport()
		return {'FINISHED'}

class B2UnfoldManuelImport(bpy.types.Operator):
	bl_idname = "b2unfoldmanual.sync"
	bl_label = "Manuel Sync"

	def execute(self, context):
		B2Unfold_ManuelImport()
		return{'FINISHED'}
...
class UnfoldUVMain(bpy.types.Panel):
	...
	def draw(self, context):
		...
		if(scn.manualTab == True):
			...
			ManualButtonExport.operator(B2UnfoldManuelExport.bl_idname, text="Send", icon="EXPORT")
			ManualButtonExport.scale_y = 1.5
		...
		if(scn.autoTab == True):
			...
			Autobutton.operator(B2Unfold.bl_idname, text="Auto Unfold!", icon='TEXTURE_DATA')
